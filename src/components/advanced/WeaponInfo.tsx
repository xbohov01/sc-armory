...
export function WeaponInfo(props: WeaponInfoProps) {
  ...
  return (
    <VStack id='weapons-info'>
      {weaponData.length === 0 ? <p>No weapons selected</p> : weaponData.map(d => <WeaponInfoDisplay key={Math.random()} weapon={d} />)}
    </VStack>
  )
}
  ...
  return (
    <VStack id="weapon-info">
      <Heading
        ...
        minWidth='200pt'
        textAlign='left'
        borderLeft='1pt solid whitesmoke' padding='5pt' bgColor='#1a2130'
      >{props.weapon.LocalizedName}</Heading>
      <Grid
        ...
        {props.weapon.SingleFireRate === 0 ? '' :
          <GridItem>
            <Stat borderLeft='1pt solid whitesmoke' padding='5pt' bgColor='#1a2130'>
              ...
              <StatNumber fontSize='md'>{props.weapon.SingleFireRate + ' RPM'}</StatNumber>
            </Stat>
          </GridItem>
        }
        {props.weapon.RapidFireRate === 0 ? '' :
          <GridItem>
            <Stat borderLeft='1pt solid whitesmoke' padding='5pt' bgColor='#1a2130'>
              ...
              <StatNumber fontSize='md'>{props.weapon.RapidFireRate + ' RPM'}</StatNumber>
            </Stat>
          </GridItem>
        }
        {props.weapon.BurstFireRate === 0 ? '' :
          <GridItem>
            <Stat borderLeft='1pt solid whitesmoke' padding='5pt' bgColor='#1a2130'>
              ...
              <StatNumber fontSize='md'>{props.weapon.BurstFireRate + ' RPM'}</StatNumber>
            </Stat>
          </GridItem>
        }
        {props.weapon.BurstFireRate === 0 ? '' :
        ...
        <GridItem colSpan={2}>
          <Accordion allowToggle width='inherit' borderLeft='1pt solid whitesmoke' padding='5pt' bgColor='#1a2130'>
            <AccordionItem>
              <h2>
                <AccordionButton>
                    <Tooltip label="In game description may differ from game file data" aria-label="A tooltip">
                      In-game description
                    </Tooltip>
                  <AccordionIcon />
                </AccordionButton>
