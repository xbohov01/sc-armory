...
export class ApiClient {
  ...
  async GetArmor(filter: string = ""): Promise<ArmorVM[]> {
    ...
    let result = await this.instance.get(this.url + ArmorsEndpoint + filter);
  ...
  async GetWeapon(filter: string = ""): Promise<WeaponVM[]> {
    ...
    let result = await this.instance.get(this.url + WeaponsEndpoint + filter);
  ...
  async GetWeaponByName(name: string): Promise<WeaponVM> {
    let result = await this.GetWeapon(`?$filter=LocalizedName eq '${name}'`);
    return result[0];
  }
